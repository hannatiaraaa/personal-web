...
| Gates | `bun run lint`, `bun run typecheck`, `bun run build`, `bun run test:e2e` |
...
**Copy is a gate, not a detail.** No page ships without `portfolio-voice` applied — the authority chain for every figure, the title policy, the confidentiality ceiling. A wrong number on a public page is the one failure in this repo that cannot be quietly fixed later, because it will have been read. New or edited `/work` pages also run `portfolio-case-study`.
