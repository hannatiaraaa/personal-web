...
test.describe('the offer is above the fold', () => {
  test('name, brand sentence, constraint and figures need no scrolling', async ({ page }) => {
    await page.goto('/');

    await expect(page.getByRole('heading', { level: 1, name: 'Hanna Tiara Andarlia' })).toBeInViewport();
    await expect(page.getByText('learned my way to the end of the pipeline')).toBeInViewport();
    await expect(page.getByText('GMT+7').first()).toBeInViewport();
    // The evidence panel, located structurally — its label is rendered twice
    // (once for assistive tech, once visually) so a text locator is ambiguous.
    await expect(page.locator('dl').first()).toBeInViewport();
  });
