  ...
  return new ImageResponse(
    <div
      style={{
        ...
        background: 'linear-gradient(140deg, #041018 0%, #062133 46%, #0b3f56 100%)',
        color: '#e9f3fd',
        padding: '72px 80px',
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={{ fontSize: 24, letterSpacing: 2, color: '#38bdf8', textTransform: 'uppercase' }}>
          {identity.brandLine}
        </div>
        <div style={{ fontSize: 82, fontWeight: 600, letterSpacing: -2, marginTop: 18, lineHeight: 1 }}>
          {identity.name}
        </div>
        <div style={{ fontSize: 30, color: '#9db4cb', marginTop: 22, maxWidth: 940, lineHeight: 1.35 }}>
          Offline-first mobile for field crews, approval and procure-to-pay workflows, and the release gate that decides
          what ships.
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 56 }}>
        {headlineFigures.map((figure) => (
          ...
          >
            ...
            <div style={{ fontSize: 20, color: '#9db4cb', marginTop: 4, maxWidth: 200 }}>{figure.label}</div>
          </div>
        ))}
      </div>

      <div style={{ fontSize: 22, color: '#6b829a' }}>
