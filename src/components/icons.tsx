...
export function EmailIcon({ className }: IconProps) {
  return (
    <svg className={`${base} ${className ?? ''}`} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="M3 6l9 6.5L21 6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  );
}
...
export function DownloadIcon({ className }: IconProps) {
  return (
    <svg className={`${base} ${className ?? ''}`} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M12 3v11m0 0l-4-4m4 4l4-4M4 19h16" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
