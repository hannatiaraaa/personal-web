  ...
  return (
    ...
    >
      {(
        ...
        <g key={name}>
          <text x="8" y={y - 32} className={diagramLabel} fill="var(--ink)">
            {name}
          </text>
          <text x="8" y={y - 16} className={diagramLabel} fill="var(--ink-faint)">
            {note}
          </text>
          <line
            x1={LANE_START}
      ...
      {/* time axis */}
      <line x1={LANE_START} y1={axis} x2={LANE_END} y2={axis} stroke="var(--line-strong)" strokeWidth="1" />
      <path d={`M${LANE_END - 6} ${axis - 4}l6 4-6 4`} stroke="var(--line-strong)" strokeWidth="1.2" strokeLinecap="round" />
      <text x="8" y={axis + 4} className={diagramLabel} fill="var(--ink-faint)">
        time
      </text>

      {/* both devices read revision 1 */}
      <line x1={readAt} y1={laneA} x2={readAt} y2={laneB} stroke="var(--line-strong)" strokeWidth="1" />
      {[laneA, laneB].map((y) => (
        <circle key={y} cx={readAt} cy={y} r="4.5" fill="var(--ink-muted)" />
      ))}
      <text x={readAt} y={laneB + 26} textAnchor="middle" className={diagramLabel} fill="var(--ink-muted)">
        both read rev 1
      </text>

      {/* the online device moves the record on */}
      <circle cx={writeAt} cy={laneB} r="4.5" fill="var(--ink)" />
      <text x={writeAt} y={laneB + 26} textAnchor="middle" className={diagramLabel} fill="var(--ink)">
        writes rev 2
      </text>

      {/* the returning device still holds a revision 1 draft */}
      <circle cx={returnAt} cy={laneA} r="4.5" fill="var(--signal)" />
      <text x={returnAt} y={laneA - 16} textAnchor="middle" className={diagramLabel} fill="var(--signal)">
      ...
      <text x={returnAt + 96} y={laneB + 4} textAnchor="middle" className={diagramLabel} fill="var(--signal)">
