...
export function TaxLadderDiagram() {
  ...
  return (
    ...
    >
      {/* before */}
      <text x={col.before} y="34" className={diagramLabel} fill="var(--ink-faint)">
      ...
      {rows.map(({ y, text }, i) => (
        <g key={text}>
          ...
          />
          <text x={col.before + 12} y={y + 18} className={diagramLabel} fill="var(--ink)">
            {text} + tax
          </text>
          {i < rows.length - 1 && (
            <path
      ...
      <text x={col.before} y="194" className={diagramLabel} fill="var(--signal)">
        tax can land on taxed money
      </text>

      {/* after */}
      <text x={col.after} y="34" className={diagramLabel} fill="var(--ink-faint)">
        AFTER — one ladder
      </text>
      {(
        [
        ...
        <g key={text}>
          ...
          />
          <text x={col.after + 12} y={y + 18} className={diagramLabel} fill="var(--ink)">
            {text}
          </text>
          {i < 2 && (
            <path
      ...
      <text x={col.after} y="194" className={diagramLabel} fill="var(--ink-muted)">
